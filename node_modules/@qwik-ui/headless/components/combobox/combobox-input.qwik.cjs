...
const useCombobox = require("./use-combobox.qwik.cjs");
const combinedRefs = require("../../hooks/combined-refs.qwik.cjs");
const HComboboxInput = qwik.component$(({ "bind:value": givenInputValueSig, ...props }) => {
  const context = qwik.useContext(comboboxContext.comboboxContextId);
  const contextRefOpts = {
    context,
    givenContextRef: context.inputRef
  };
  const inputRef = combinedRefs.useCombinedRef(props.ref, contextRefOpts);
  ...
  const initialValueSig = qwik.useSignal();
  const wasEmptyBeforeBackspaceSig = qwik.useSignal(false);
  const isInputResetSig = qwik.useSignal(false);
  const { selectionManager$, getNextEnabledItemIndex$, getPrevEnabledItemIndex$ } = useCombobox.useCombobox();
  ...
  const handleKeyDown$ = qwik.$(async (e) => {
    ...
    const handleArrowNavigation$ = qwik.$(async (getEnabledItemIndex$) => {
  ...
  const handleInput$ = qwik.$(async (e, el) => {
  ...
  const handleKeyUp$ = qwik.$((e) => {
    if (e.key === "Backspace") {
      if (!context.multiple) return;
      if (context.selectedValuesSig.value.length === 0) return;
      if (!context.removeOnBackspace) return;
      if ((wasEmptyBeforeBackspaceSig.value || isInputResetSig.value) && context.inputValueSig.value.length === 0) {
  ...
  qwik.useTask$(function inlineModeInit() {
    if (context.mode === "inline") {
      context.highlightedIndexSig.value = 0;
    }
  });
  qwik.useTask$(function getInitialValues() {
    ...
    for (const [index, item] of itemsMapSig.value.entries()) {
      const isSelected = multiple ? Array.isArray(selectedValues) && selectedValues.includes(item.value) : item.value === selectedValues;
      if (isSelected) {
  ...
  const inputValueSig = qwik.useComputed$(() => givenInputValueSig?.value ?? initialValueSig.value ?? "");
  return /* @__PURE__ */ jsxRuntime.jsx("input", {
    ...
    onKeyUp$: [
      context.removeOnBackspace ? handleKeyUp$ : void 0,
      props.onKeyUp$
    ],
    onInput$: [
      handleInput$,
    ...
    "aria-haspopup": context.mode === "inline" ? void 0 : "listbox",
    ref: inputRef,
    autocomplete: "off",
    placeholder: context.placeholder ?? props.placeholder ?? void 0,
    "data-combobox-input": true,
    "data-invalid": context.isInvalidSig.value ? "" : void 0,
    ...props
  });
});
exports.HComboboxInput = HComboboxInput;
