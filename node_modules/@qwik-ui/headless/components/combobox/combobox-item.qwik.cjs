"use strict";
Object.defineProperty(exports, Symbol.toStringTag, { value: "Module" });
const jsxRuntime = require("@builder.io/qwik/jsx-runtime");
const qwik = require("@builder.io/qwik");
const comboboxContext = require("./combobox-context.qwik.cjs");
const useCombobox = require("./use-combobox.qwik.cjs");
const build = require("@builder.io/qwik/build");
const combinedRefs = require("../../hooks/combined-refs.qwik.cjs");
const HComboboxItem = qwik.component$((props) => {
  const context = qwik.useContext(comboboxContext.comboboxContextId);
  const debounceTimeoutSig = qwik.useSignal();
  if (props._index === void 0) {
    throw new Error("Qwik UI: Combobox component item cannot find its proper index.");
  }
  const localIndexSig = qwik.useComputed$(() => props._index ?? -1);
  const itemRef = combinedRefs.useCombinedRef(props.ref);
  const { selectionManager$, filterManager$ } = useCombobox.useCombobox();
  const itemLabelId = `${context.localId}-${localIndexSig.value}-item-label`;
  const isDisabledSig = qwik.useComputed$(() => context.disabledIndexSetSig.value.has(localIndexSig.value));
  const isSelectedSig = qwik.useComputed$(() => {
    const index = localIndexSig.value;
    const selectedValue = context.itemsMapSig.value.get(index)?.value;
    if (!selectedValue || isDisabledSig.value) return false;
    if (Array.isArray(context.selectedValuesSig.value)) {
      return context.selectedValuesSig.value.includes(selectedValue);
    } else {
      return context.selectedValuesSig.value === selectedValue;
    }
  });
  const isHighlightedSig = qwik.useComputed$(() => {
    if (isDisabledSig.value) return;
    if (context.highlightedIndexSig.value === localIndexSig.value) {
      return true;
    } else {
      return false;
    }
  });
  const checkVisibility$ = qwik.$(async (entries) => {
    const [entry] = entries;
    if (isHighlightedSig.value && !entry.isIntersecting) {
      const containerRect = context.panelRef.value?.getBoundingClientRect();
      const itemRect = itemRef.value?.getBoundingClientRect();
      if (!containerRect || !itemRect || !context.isKeyboardFocusSig.value) return;
      const offset = itemRect.top - containerRect.top - containerRect.height / 2 + itemRect.height / 2;
      context.panelRef.value?.scrollBy({
        top: document.hasFocus() ? offset : void 0,
        ...context.scrollOptions
      });
    }
  });
  const handleClick$ = qwik.$(async () => {
    if (isDisabledSig.value || localIndexSig.value === null) return;
    await selectionManager$(localIndexSig.value, "toggle");
    if (!isSelectedSig.value && !context.multiple) {
      context.isListboxOpenSig.value = false;
    }
  });
  const handlePointerOver$ = qwik.$(() => {
    if (isDisabledSig.value) return;
    if (props._index !== void 0 && (context.isMouseOverPopupSig.value || context.mode === "inline")) {
      context.highlightedIndexSig.value = localIndexSig.value;
    }
  });
  const handleFocus$ = qwik.$(() => {
    context.inputRef.value?.focus();
  });
  const itemContext = {
    isSelectedSig,
    itemLabelId,
    _index: props._index
  };
  qwik.useContextProvider(comboboxContext.comboboxItemContextId, itemContext);
  qwik.useTask$(function handleScrolling({ track, cleanup }) {
    track(() => context.highlightedIndexSig.value);
    if (build.isServer || !context.panelRef.value) return;
    const hasScrollbar = context.panelRef.value.scrollHeight > context.panelRef.value.clientHeight;
    if (!hasScrollbar) return;
    if (debounceTimeoutSig.value !== void 0) {
      clearTimeout(debounceTimeoutSig.value);
    }
    debounceTimeoutSig.value = setTimeout(() => {
      if (props._index !== context.highlightedIndexSig.value) return;
      const observer = new IntersectionObserver(checkVisibility$, {
        root: context.panelRef.value,
        threshold: 0
      });
      cleanup(() => observer?.disconnect());
      if (itemRef.value) {
        observer.observe(itemRef.value);
      }
    }, 100);
    cleanup(() => {
      if (debounceTimeoutSig.value !== void 0) {
        clearTimeout(debounceTimeoutSig.value);
      }
    });
  });
  qwik.useTask$(async function defaultFilter({ track }) {
    track(() => context.inputValueSig.value);
    if (build.isServer || !itemRef.value) return;
    const displayValue = context.itemsMapSig.value.get(localIndexSig.value)?.displayValue;
    let isVisible;
    if (!displayValue) return;
    if (context.filter) {
      const lowerCaseDisplayValue = displayValue?.toLowerCase();
      const lowerCaseInputValue = context.inputValueSig.value.toLowerCase();
      isVisible = lowerCaseDisplayValue?.includes(lowerCaseInputValue);
      filterManager$(!!isVisible, itemRef, localIndexSig.value);
    }
  });
  qwik.useTask$(({ track }) => {
    track(() => context.isListboxOpenSig.value);
    if (context.isListboxOpenSig.value) {
      context.initialLoadSig.value = false;
    }
  });
  return /* @__PURE__ */ jsxRuntime.jsx("div", {
    role: "option",
    ref: itemRef,
    tabIndex: -1,
    id: `${context.localId}-${localIndexSig.value}`,
    "aria-selected": isSelectedSig.value,
    "aria-disabled": isDisabledSig.value === true ? "true" : "false",
    "data-disabled": isDisabledSig.value ? "" : void 0,
    "data-selected": isSelectedSig.value ? "" : void 0,
    "data-highlighted": isHighlightedSig.value ? "" : void 0,
    onPointerOver$: [
      handlePointerOver$,
      props.onPointerOver$
    ],
    onFocus$: [
      handleFocus$,
      props.onFocus$
    ],
    "aria-labelledby": itemLabelId,
    "data-item": true,
    onClick$: [
      handleClick$,
      props.onClick$
    ],
    ...props,
    children: /* @__PURE__ */ jsxRuntime.jsx(qwik.Slot, {})
  });
});
exports.HComboboxItem = HComboboxItem;
