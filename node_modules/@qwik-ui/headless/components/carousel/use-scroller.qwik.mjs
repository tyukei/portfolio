...
function useScroller(context) {
  const startPosSig = useSignal();
  const transformSig = useSignal({
    x: 0,
    y: 0,
    z: 0
  });
  const boundariesSig = useSignal(null);
  const isMouseDownSig = useSignal(false);
  const isTouchDeviceSig = useSignal(false);
  const isInitialTransitionSig = useSignal(true);
  const givenTransitionSig = useSignal();
  useTask$(() => {
